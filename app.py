...
if page=="📊 Overview":
    ...
    # Correct savings: project Period A rates onto Period B hours
    _sv = 0
    for _nm, _cl, _iv in [('VIP-11','vip11power',1),('VIP-12','vip12power',1),('RJC-01','rjc1curr',0),('RJC-02','rjc2curr',0),('RJC-03','rjc3curr',0)]:
        if _iv: _proj = pa_run[_cl].mean()*pb_rh + pa_stp[_cl].mean()*(len(pb[pb['linespeed']==0])*5/60)
        else: _proj = (pa_run[_cl].mean()/100*250)*pb_rh
        _sv += _proj - sb_d[fc(sb_d,_nm)].sum()
    c1,c2,c3,c4,c5=st.columns(5)
    c1.metric("Annual Saving",f"₹ {_sv*5/1e7:.2f} Cr"); c2.metric("Energy Saved",f"{_sv/1000:,.0f} MWh")
