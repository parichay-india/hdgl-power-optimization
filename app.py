...
if not os.path.exists(DATA_FILE):
    st.error("Dataset not found. Please check Google Drive link.")
    st.stop()
...
# PRECOMPUTE
# ============================================================
pa_run = pa[pa['linespeed'] > 0]
pb_run = pb[pb['linespeed'] > 0]
pa_stp = pa[pa['linespeed'] == 0]
sa_d = sa[sa['Sl.No.'].notna()].copy()
sb_d = sb[sb['Sl.No.'].notna()].copy()

def fc(df, kw):
    return [c for c in df.columns if kw in c][0]

pa_rh = len(pa_run) * 5 / 60
pb_rh = len(pb_run) * 5 / 60
gt_a = fc(sa_d, 'Grand Total')
gt_b = fc(sb_d, 'Grand Total')
...
if page == "📊 Overview":
    ...
    c1.metric("Annual Saving", f"₹ {sv*5/1e7:.2f} Cr")
    c2.metric("Energy Saved", f"{sv/1000:,.0f} MWh")
    c3.metric("SEC Reduction", "32.2 → 8.1", "−74.8% kWh/ton")
    c4.metric("VIP Power", f"{pb_run['vip11power'].mean():.0f} kW/ea", f"was {pa_run['vip11power'].mean():.0f}")
    ...
    with cl:
        ...
        ma = sa_d.groupby('_m')[gt_a].sum().reset_index()
        mb = sb_d.groupby('_m')[gt_b].sum().reset_index()
        fig = go.Figure()
        fig.add_bar(x=ma['_m'], y=ma[gt_a]/1000, name='Period A (Legacy)', marker_color=CA)
        fig.add_bar(x=mb['_m'], y=mb[gt_b]/1000, name='Period B (AI)', marker_color=CB)
        fig.update_layout(**PL, barmode='group', height=420, title='Monthly Power Consumption (MWh)', yaxis_title='MWh', legend=dict(orientation='h', y=1.12))
        st.plotly_chart(fig, use_container_width=True)
    with cr:
        comps = ['VIP-11', 'VIP-12', 'RJC-01', 'RJC-02', 'RJC-03']
        sv_c = [(sa_d[fc(sa_d, c)].sum() - sb_d[fc(sb_d, c)].sum())/1000 for c in comps]
        fig = go.Figure()
        fig.add_bar(x=comps, y=sv_c, marker_color=['#E53935','#E53935','#1565C0','#1565C0','#1565C0'], text=[f'{v:,.0f}' for v in sv_c], textposition='outside')
        fig.update_layout(**PL, height=420, title='Saving by Component (MWh)', yaxis_title='MWh')
        st.plotly_chart(fig, use_container_width=True)
    st.divider()
    st.markdown("### Operating Parameters")
    st.dataframe(pd.DataFrame({
        'Parameter': ['Running Hours', 'Production', 'VIP Power/ea', 'RJC-01', 'RJC-02', 'RJC-03', 'RJC Target', 'SEC'],
        ...
        'Period B': [f'{pb_rh:,.1f}h', '2,40,453 t', f'{pb_run["vip11power"].mean():.1f} kW', f'{pb_run["rjc1curr"].mean():.1f}%', f'{pb_run["rjc2curr"].mean():.1f}%', f'{pb_run["rjc3curr"].mean():.1f}%', '465-470°C', '8.1 kWh/t']
...
# ============================================================
    ...
    with tab1:
        ...
        vip = st.radio("", ['VIP-11', 'VIP-12'], horizontal=True)
        col = 'vip11power' if '11' in vip else 'vip12power'
        fig = go.Figure()
        fig.add_histogram(x=pa_run[col], name='A', opacity=.55, marker_color='#E53935', nbinsx=80)
        fig.add_histogram(x=pb_run[col], name='B', opacity=.55, marker_color='#1565C0', nbinsx=80)
        ...
        rf.update_layout(**PL, height=350, barmode='overlay')
        st.plotly_chart(rf, use_container_width=True)
    with tab2:
        st.markdown("### Zone Temperatures (Running)")
        zn = ['DFF', 'RTH', 'RTS', 'RJC', 'Snout']
        cm = ['dffstrip_ACT', 'rthstrip_ACT', 'rtsstrip_ACT', 'rjcstrip_ACT', 'snoutstrip_ACT']
        at = [pa_run[pa_run[c] < 1900][c].mean() for c in cm]
        bt = [pb_run[pb_run[c] < 1900][c].mean() for c in cm]
        fig = go.Figure()
        fig.add_bar(x=zn, y=at, name='A', marker_color='#E53935', text=[f'{v:.0f}°C' for v in at], textposition='outside')
    ...
    with tab3:
        ...
        pc = ca2.radio("Period", ['A', 'B'], horizontal=True)
        ds = pa if pc == 'A' else pb
        ds = ds.copy()
        ds['_d'] = ds['datetime'].dt.date
        gd = ds.groupby('_d')['linespeed'].apply(lambda x: (x > 0).sum())
        gd = gd[gd > 150].index.tolist()
        sd = cb2.selectbox("Day", gd[:50])
        ...
        for i, v in enumerate(vs):
            sec = v == 'linespeed'
            fig.add_scatter(x=dd['datetime'], y=dd[v], name=v, line=dict(color=cc[i % 6], width=1.5 if not sec else 1, dash='dot' if sec else 'solid'), secondary_y=sec)
        fig.update_layout(**PL, height=450, title=f'{sd}')
        st.plotly_chart(fig, use_container_width=True)
    with tab4:
        st.markdown("### Correlation Matrix")
        pr = st.radio("", ['A', 'B'], horizontal=True, key='cx')
        dx = pa_run if pr == 'A' else pb_run
...
# ============================================================
    ...
    with tab1:
        ...
        st.dataframe(pd.DataFrame({
            'Layer': ['Input', 'BiGRU', 'LSTM', 'Dense', 'Dropout', 'Dense', 'Output'],
            'Config': ['(12,19)', '64/dir→128', '128 units', '64,ReLU', '0.2', '32,ReLU', '7,Linear'],
            'Shape': ['(12,19)', '(12,128)', '(128,)', '(64,)', '(64,)', '(32,)', '(7,)'],
            'Purpose': ['1hr×19 features', 'Forward+backward', 'Thermal memory', 'Compression', 'Regularize', 'Compress', 'Setpoints']
        }).set_index('Layer'), use_container_width=True)
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Params", "~148K")
        c2.metric("Inference", "<200ms")
        c3.metric("Lookback", "1 hour")
        c4.metric("Retrain", "Quarterly")
        st.markdown("### Hyperparameters")
        st.dataframe(pd.DataFrame({
            'Param': ['Optimizer', 'LR', 'Batch', 'Loss', 'EarlyStop', 'LR Sched', 'Precision', 'GPU'],
            'Value': ['Adam', '0.001', '128', 'MSE', 'patience=20', 'ReduceLR(0.5,p=10)', 'float16', 'RTX 3050 4GB']
        }).set_index('Param'), use_container_width=True)
    with tab2:
        st.markdown("### Setpoint Distributions")
        z = st.selectbox("Zone", ['rjcstrip_SP', 'snoutstrip_SP', 'dffstrip_SP', 'rthstrip_SP', 'rtsstrip_SP'])
        fig = go.Figure()
        fig.add_histogram(x=pa_run[z], name='A', opacity=.55, marker_color='#E53935', nbinsx=50)
        fig.add_histogram(x=pb_run[z], name='B', opacity=.55, marker_color='#1565C0', nbinsx=50)
        fig.update_layout(**PL, barmode='overlay', height=400, title=z, xaxis_title='°C')
        st.plotly_chart(fig, use_container_width=True)
    with tab3:
        st.markdown("### SP vs Actual Tracking")
        z2 = st.selectbox("Zone", ['DFF', 'RTH', 'RTS', 'RJC', 'Snout'], key='sa')
        base = {'DFF': 'dffstrip', 'RTH': 'rthstrip', 'RTS': 'rtsstrip', 'RJC': 'rjcstrip', 'Snout': 'snoutstrip'}[z2]
        s = st.slider("Start", 0, len(pb_run)-600, 5000, 500)
        w = st.slider("Window", 100, 1000, 400, 50)
        sm = pb_run.iloc[s:s+w]
        fig = go.Figure()
        ...
        fig.update_layout(**PL, height=400, title=f'{z2}: SP vs ACT', yaxis_title='°C')
        st.plotly_chart(fig, use_container_width=True)
...
# ============================================================
    ...
    for nm, cl, iv in [('VIP-11', 'vip11power', 1), ('VIP-12', 'vip12power', 1), ('RJC-01', 'rjc1curr', 0), ('RJC-02', 'rjc2curr', 0), ('RJC-03', 'rjc3curr', 0)]:
        if iv:
            proj = pa_run[cl].mean() * pb_rh + pa_stp[cl].mean() * (len(pb[pb['linespeed'] == 0]) * 5 / 60)
        else:
            proj = (pa_run[cl].mean() / 100 * 250) * pb_rh
        act = sb_d[fc(sb_d, nm)].sum()
        sv2 = proj - act
        res.append({'Equipment': nm, 'Projected': int(proj), 'Actual': int(act), 'Saving_kWh': int(sv2), 'Saving_Rs': int(sv2 * rate)})
    rd = pd.DataFrame(res)
    ts = rd['Saving_kWh'].sum()
    tr = rd['Saving_Rs'].sum()
    ...
    st.divider()
    dd2 = rd.copy()
    dd2['Projected'] = dd2['Projected'].apply(lambda x: f"{x:,}")
    dd2['Actual'] = dd2['Actual'].apply(lambda x: f"{x:,}")
    dd2['Saving_kWh'] = dd2['Saving_kWh'].apply(lambda x: f"{x:,}")
    dd2['Saving_Rs'] = dd2['Saving_Rs'].apply(lambda x: f"₹{x:,}")
    st.dataframe(dd2.set_index('Equipment'), use_container_width=True)
    ...
    with cr2:
        fig = go.Figure()
        fig.add_bar(x=rd['Equipment'], y=[v/1e5 for v in rd['Saving_Rs']], marker_color=['#E53935', '#E53935', '#1565C0', '#1565C0', '#1565C0'], text=[f"₹{v/1e5:.0f}L" for v in rd['Saving_Rs']], textposition='outside')
        fig.update_layout(**PL, height=380, title='Saving (₹ Lakh)', yaxis_title='₹ Lakh')
        st.plotly_chart(fig, use_container_width=True)
...
# ============================================================
    ...
    st.divider()
    c1, c2, c3, c4 = st.columns(4)
    th = c1.slider("Thickness mm", .30, 2.00, .79, .01)
    wd = c2.selectbox("Width mm", [907, 1007, 1127, 1227, 1252, 1407, 1507], index=3)
    sp = c3.slider("Speed mpm", 0, 180, 100, 5)
    por = c4.slider("POR Rest m", 0, 2500, 1200, 50)
    ...
    if sp == 0:
        ...
        thin = th <= 1.0
        tgt = 470.0 if thin else 465.0
        dff, rth, rts = 660.0, 760.0, 760.0
        ww = False
        if por < 500:
            r = (500 - por) / 500 * 15
            dff -= r
            ...
            rts -= r * .5
            ww = True
        sa_adj = {'Summer': -3, 'Monsoon': 0, 'Winter': 3}
        vip = max(45, min(80, 55 + (460 - tgt) * 5 + sa_adj[ssn]))
        cd = 15 + th * wd / 1000 * 5 + sp / 120 * 10
        r3 = min(45, max(15, cd * 1.2))
        r1 = min(35, max(0, cd * .6))
        r2 = max(0, cd * .15)
        tai = vip * 2 + (r3 + r1 + r2) / 100 * 250
        tleg = 302.5 * 2 + 75 * 3
        if ww:
            st.warning(f"⚠️ Weld Approaching — POR:{por}m, setpoints reduced {(500-por)/500*15:.1f}°C")
        cl3, cr3 = st.columns(2)
        with cl3:
            st.markdown("### 🌡️ Setpoints")
            st.dataframe(pd.DataFrame({
                'Zone': ['DFF', 'RTH', 'RTS', 'RJC', 'Snout'],
                'SP (°C)': [f'{dff:.1f}', f'{rth:.1f}', f'{rts:.1f}', f'{tgt:.0f}', f'{tgt:.0f}'],
                'Logic': ['AI' + (' ⚠️' if ww else '')] * 3 + [f'{"Thin" if thin else "Thick"}', '=RJC']
            }).set_index('Zone'), use_container_width=True)
        with cr3:
            st.markdown("### ⚡ Power")
            st.dataframe(pd.DataFrame({
                'Equip': ['VIP-11', 'VIP-12', 'RJC-03', 'RJC-01', 'RJC-02'],
                'Power': [f'{vip:.0f}kW', f'{vip:.0f}kW', f'{r3:.0f}%={r3*2.5:.0f}kW', f'{r1:.0f}%={r1*2.5:.0f}kW', f'{r2:.0f}%={r2*2.5:.0f}kW'],
                'Status': ['🟢', '🟢', '🟢 Base', '🟢' if r1 > 2 else '🟡', '🟢' if r2 > 2 else '🟡']
            }).set_index('Equip'), use_container_width=True)
        st.divider()
        c1, c2, c3 = st.columns(3)
        c1.metric("AI Power", f"{tai:.0f} kW")
        c2.metric("Legacy", f"{tleg:.0f} kW")
        c3.metric("Saving", f"{tleg-tai:.0f} kW", f"−{(1-tai/tleg)*100:.0f}%")
        fig = go.Figure(go.Indicator(
            mode="gauge+number+delta", value=tai,
            delta={'reference': tleg, 'decreasing': {'color': '#00B050'}},
            gauge={'axis': {'range': [0, 1000]}, 'bar': {'color': '#1565C0'},
                   'steps': [{'range': [0, 250], 'color': '#C8E6C9'}, {'range': [250, 500], 'color': '#FFF9C4'}, {'range': [500, 1000], 'color': '#FFCDD2'}],
